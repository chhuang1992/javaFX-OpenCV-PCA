...
public class UsePCATest {
	...
	public static double getOrientation(MatOfPoint pts_, Mat img){
		System.out.println("Input MatOfPoint's row number is "+pts_.rows());
		System.out.println("Input MatOfPoint's size number is "+pts_.size());
		
		Point[] pts = pts_.toArray();
		int sz = pts.length;
		System.out.println("Trans Input MatOfPoint to Array,the Array's length: "+sz);
		...
		Mat vectors = new Mat();
		Mat covar = new Mat();
		Mat data_pts_t = new Mat();
		Mat data_pts_mul = new Mat();
		...
		Core.transpose(data_pts, data_pts_t);
		System.out.println("The InputData's transpose:");
		System.out.println(data_pts_t.dump());
		
		Core.gemm(data_pts_t, data_pts, 1.0/(double)sz, new Mat(2,2,CvType.CV_64FC1), 0, data_pts_mul); ;
		System.out.println("The InputData's transpose * The InputData *(1/The InputData's row number):(we use gemm to compute)");
		System.out.println(data_pts_mul.dump());
		
		Core.eigen(data_pts_mul, true, eigenvalues, eigenvectors);
		System.out.println("The InputData's multiply's eigenvector and eigenvalue:");
		...
		System.out.println("The InputData's Covariance Matrix:");
	...
	public static void main(String[] args) {
	    ...
	    //THIS IS USED TO TEST Core.line AND drawAxis
	    //Point A = new Point(50,50);
		//Point B = new Point(350,50);
		//Scalar colour = new Scalar(255,0,0) ;
		//Core.line(img, A, B, colour, 1, Core.LINE_AA,0);
		//drawAxis(img,A,B,colour,0.2);
		//Highgui.imwrite("data/picLine.jpg", img);
